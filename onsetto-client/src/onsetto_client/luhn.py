...
def luhn_checksum(digits: str) -> int:
    """Return the Luhn check digit for a string of digits (excluding
    the check digit itself).

    NOTE: Algorithm from https://en.wikipedia.org/wiki/Luhn_algorithm

    Args:
        digits (str): The card digits to check

    Raise:
        ValueError if there are non-digits in the passed str

    Return:
        the int Luhn check digit
    """
...
def luhn_is_valid(card_number: str) -> bool:
    """See if card_number passes the Luhn check.
...
def luhn_generate(prefix: str = "4", length: int = 16) -> str:
    """Generate a Luhn-valid card number with the given prefix and total
    length. Fills (length - 1) positions with prefix + random digits, then
    computes and appends the Luhn check digit.

    Args:
        prefix (str): The first digit(s) of the generated card number
        length (int): The length of the generated card number

    Raise:
        ValueError if the prefix is empty or not a digit, or if the length
        provided is shorter than the lenght of the prefix + 1

    Return:
        str generated test card number
    """
